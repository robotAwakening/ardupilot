...
void AP_Proximity_RPLidarA2::get_readings()
{
    ...
    while (_uart->available() && _bytes_read < MAX_BYTES_CONSUME) {
        ...
        case State::AWAITING_RESPONSE:
            ...
            if ((_read_result = _uart->read(&_payload[0], sizeof(_descriptor))) > 0) {
        ...
        case State::AWAITING_SCAN_DATA:
            ...
            if ((_read_result = _uart->read(&_payload[0], sizeof(_payload.sensor_scan))) > 0) {
                _bytes_read += _read_result;
            } else {
                return;
            }
        ...
        case State::AWAITING_HEALTH:
            ...
            if ((_read_result = _uart->read(&_payload[0], sizeof(_payload.sensor_health))) > 0) {
                _bytes_read += _read_result;
            } else {
                return;
            }
